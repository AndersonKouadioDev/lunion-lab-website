...
// Magic UI — Safari (mockup navigateur)
...
export function Safari({
  ...
  // IDs uniques par instance (le carrousel rend plusieurs Safari → éviter les IDs SVG dupliqués qui cassent le masque)
  const uid = useId().replace(/:/g, "")
  const punchId = `safariPunch-${uid}`
  const clipId = `safariClip-${uid}`
  const roundId = `safariRound-${uid}`
