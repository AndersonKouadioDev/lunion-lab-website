...
export function Iphone({
  ...
  // IDs uniques par instance (carrousel = plusieurs iPhone → éviter les IDs SVG dupliqués)
  const uid = useId().replace(/:/g, "")
  const screenId = `iphoneScreen-${uid}`
  const cornerId = `iphoneCorner-${uid}`
  ...
  return (
    ...
    >
      ...
      >
        <g mask={hasMedia ? `url(#${screenId})` : undefined}>
          <path
        ...
        <path
          ...
          mask={hasMedia ? `url(#${screenId})` : undefined}
        />
        ...
        <defs>
          <mask id={screenId} maskUnits="userSpaceOnUse">
            <rect
              x="0"
              y="0"
              width={PHONE_WIDTH}
